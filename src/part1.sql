...
CREATE OR REPLACE FUNCTION trigger_checks_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('checks_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы checks на insert
CREATE TRIGGER trigger_checks_insert BEFORE
INSERT ON checks FOR EACH ROW EXECUTE FUNCTION trigger_checks_insert();
-- Триггерная функция для таблицы p2p на insert
CREATE OR REPLACE FUNCTION trigger_p2p_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('p2p_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы p2p на insert
CREATE TRIGGER trigger_p2p_insert BEFORE
INSERT ON p2p FOR EACH ROW EXECUTE FUNCTION trigger_p2p_insert();
-- Триггерная функция для таблицы verter на insert
CREATE OR REPLACE FUNCTION trigger_verter_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('verter_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы verter на insert
CREATE TRIGGER trigger_verter_insert BEFORE
INSERT ON verter FOR EACH ROW EXECUTE FUNCTION trigger_verter_insert();
-- Триггерная функция для таблицы transferred_points на insert
CREATE OR REPLACE FUNCTION trigger_transferred_points_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('transferred_points_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы transferred_points на insert
CREATE TRIGGER trigger_transferred_points_insert BEFORE
INSERT ON transferred_points FOR EACH ROW EXECUTE FUNCTION trigger_transferred_points_insert();
-- Триггерная функция для таблицы friends на insert
CREATE OR REPLACE FUNCTION trigger_friends_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('friends_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы friends на insert
CREATE TRIGGER trigger_friends_insert BEFORE
INSERT ON friends FOR EACH ROW EXECUTE FUNCTION trigger_friends_insert();
-- Триггерная функция для таблицы recommendations на insert
CREATE OR REPLACE FUNCTION trigger_recommendations_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('recommendations_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы recommendations на insert
CREATE TRIGGER trigger_recommendations_insert BEFORE
INSERT ON recommendations FOR EACH ROW EXECUTE FUNCTION trigger_recommendations_insert();
-- Триггерная функция для таблицы xp на insert
CREATE OR REPLACE FUNCTION trigger_xp_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('xp_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы xp на insert
CREATE TRIGGER trigger_xp_insert BEFORE
INSERT ON xp FOR EACH ROW EXECUTE FUNCTION trigger_xp_insert();
-- Триггерная функция для таблицы time_tracking на insert
CREATE OR REPLACE FUNCTION trigger_time_tracking_insert() RETURNS TRIGGER AS $ $ BEGIN NEW.id := nextval('time_tracking_id_seq');
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Триггер для таблицы time_tracking на insert
CREATE TRIGGER trigger_time_tracking_insert BEFORE
INSERT ON time_tracking FOR EACH ROW EXECUTE FUNCTION trigger_time_tracking_insert();
...
CREATE OR REPLACE FUNCTION check_parent_task() RETURNS TRIGGER AS $ $ BEGIN IF NEW.title != 'Pool'
    AND NEW.parent_task IS NULL THEN RAISE EXCEPTION 'parent_task must be not null';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что parent_task не может быть null (кроме "Pool")
CREATE TRIGGER trigger_check_parent_task BEFORE
INSERT
    OR
UPDATE ON tasks FOR EACH ROW EXECUTE FUNCTION check_parent_task();
...
CREATE OR REPLACE FUNCTION check_parent_task_before_delete() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
        SELECT 1
        FROM tasks
        WHERE parent_task = OLD.title
    ) THEN RAISE EXCEPTION 'cannot delete task that is a parent task for other tasks';
END IF;
RETURN OLD;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет перед удалением записи, что title не является для какой-либо другой записи parent_task
CREATE TRIGGER trigger_check_parent_task_before_delete BEFORE DELETE ON tasks FOR EACH ROW EXECUTE FUNCTION check_parent_task_before_delete();
...
CREATE OR REPLACE FUNCTION check_date_p2p() RETURNS TRIGGER AS $ $ BEGIN IF NOT EXISTS(
        SELECT 1
        FROM checks
        WHERE id = NEW.check_id
            AND date_check = NEW.time_check::date
    ) THEN RAISE EXCEPTION 'new record in p2p must have the same date in checks';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что добавляемая в p2p проверка имеет такую же дату, что и в checks
CREATE TRIGGER trigger_check_date_p2p BEFORE
INSERT
    OR
UPDATE ON p2p FOR EACH ROW EXECUTE FUNCTION check_date_p2p();
...
CREATE OR REPLACE FUNCTION check_state_first_record_in_p2p() RETURNS TRIGGER AS $ $ BEGIN IF NOT EXISTS(
        SELECT 1
        FROM p2p
        WHERE check_id = NEW.check_id
    ) THEN IF NEW.state_check != 'start' THEN RAISE EXCEPTION 'only records with state "start" are allowed when check_id does not exist in p2p table';
END IF;
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что первая запись в p2p для соответствующей проверки должна иметь статус 'start'
CREATE TRIGGER check_state_first_record_in_p2p BEFORE
INSERT
    OR
UPDATE ON p2p FOR EACH ROW EXECUTE FUNCTION check_state_first_record_in_p2p();
...
CREATE OR REPLACE FUNCTION check_time_second_record_in_p2p() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
        SELECT 1
        FROM p2p
        WHERE state_check = 'start'
            AND check_id = NEW.check_id
            AND NEW.time_check <= time_check
    ) THEN RAISE EXCEPTION 'time_check for the new p2p record should be after start record';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что добавляемая в p2p запись не раньше чем запись start для соответствующей проверки
CREATE TRIGGER trigger_check_time_second_record_in_p2p BEFORE
INSERT
    OR
UPDATE ON p2p FOR EACH ROW EXECUTE FUNCTION check_time_second_record_in_p2p();
...
CREATE OR REPLACE FUNCTION check_verter_date() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
        SELECT 1
        FROM checks
        WHERE id = NEW.check_id
            AND date_check != NEW.time_check::date
    ) THEN RAISE EXCEPTION 'the record must have the same date in checks table';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что проверка verter'ом проходит в тот же день, что и p2p
CREATE TRIGGER trigger_check_verter_date BEFORE
INSERT
    OR
UPDATE ON verter FOR EACH ROW EXECUTE FUNCTION check_verter_date();
...
CREATE OR REPLACE FUNCTION check_state_first_record_in_verter() RETURNS TRIGGER AS $ $ BEGIN IF NOT EXISTS(
        SELECT 1
        FROM verter
        WHERE check_id = NEW.check_id
    ) THEN IF NEW.state_check != 'start' THEN RAISE EXCEPTION 'only records with state "start" are allowed when check_id does not exist in verter table';
END IF;
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что первая запись в verter имеет статус 'start'
CREATE TRIGGER check_state_first_record_in_verter BEFORE
INSERT
    OR
UPDATE ON verter FOR EACH ROW EXECUTE FUNCTION check_state_first_record_in_verter();
...
CREATE OR REPLACE FUNCTION check_verter_time() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
    ...
    ) THEN RAISE EXCEPTION 'verter checking time cannot be earlier than p2p checking time';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что время проверки verter'ом не раньше, чем окончание проверки p2p
CREATE TRIGGER check_verter_time_trigger BEFORE
INSERT
    OR
UPDATE ON verter FOR EACH ROW EXECUTE FUNCTION check_verter_time();
...
CREATE OR REPLACE FUNCTION check_time_second_record_in_verter() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
        SELECT time_check
        FROM verter
        WHERE state_check = 'start'
            AND NEW.check_id = check_id
            AND NEW.time_check <= time_check
    ) THEN RAISE EXCEPTION 'time_check for the new verter record should be after start record';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что добавляемая в verter запись не раньше чем запись start для соответствующей проверки
CREATE TRIGGER trigger_check_time_second_record_in_verter BEFORE
INSERT
    OR
UPDATE ON verter FOR EACH ROW EXECUTE FUNCTION check_time_second_record_in_verter();
...
CREATE OR REPLACE FUNCTION check_recommendation() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
        SELECT 1
        FROM transferred_points
        WHERE checked_peer = NEW.peer
            AND checking_peer = NEW.recommended_peer
    ) THEN RETURN NEW;
ELSE RAISE EXCEPTION 'recommended_peer must be a checking_peer of the corresponding checked_peer';
END IF;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что рекомендуемый пир проводил проверку рекомендующего
CREATE TRIGGER trigger_check_recommendation BEFORE
INSERT
    OR
UPDATE ON recommendations FOR EACH ROW EXECUTE FUNCTION check_recommendation();
...
DECLARE last_state integer;
BEGIN
SELECT state_track INTO last_state
FROM time_tracking
...
LIMIT 1;
IF last_state = 1
AND NEW.state_track = 1 THEN RAISE EXCEPTION 'state 2 entry is missing before state 1 entry in time_tracking';
END IF;
IF (
    last_state = 2
    OR last_state IS NULL
)
AND NEW.state_track = 2 THEN RAISE EXCEPTION 'state 1 entry is missing before state 2 entry in time_tracking';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет корректность записей в time_tracking
CREATE TRIGGER trigger_check_time_tracking BEFORE
INSERT
    OR
UPDATE ON time_tracking FOR EACH ROW EXECUTE FUNCTION check_time_tracking();
...
DECLARE count_state_1 integer;
count_state_2 integer;
BEGIN
SELECT COUNT(*) INTO count_state_1
FROM time_tracking
WHERE peer_nickname = OLD.peer_nickname
    AND date_track = OLD.date_track
    AND state_track = 1;
SELECT COUNT(*) INTO count_state_2
FROM time_tracking
WHERE peer_nickname = OLD.peer_nickname
    AND date_track = OLD.date_track
    AND state_track = 2;
IF count_state_1 != count_state_2 THEN RAISE EXCEPTION 'In time_tracking, the number of state 1 and state 2 entries must be equal for each peer and date';
END IF;
RETURN OLD;
END;
$ $ LANGUAGE plpgsql;
...
AFTER DELETE ON time_tracking FOR EACH ROW EXECUTE FUNCTION check_time_tracking_before_delete();
...
CREATE OR REPLACE FUNCTION check_parent_task_in_xp() RETURNS trigger AS $ $ BEGIN IF (
        SELECT parent_task
        FROM tasks
    ...
    ) THEN RAISE EXCEPTION 'parent task is not completed';
END IF;
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет на завершение родительского таска для добавляемой записи проверки в таблицу check
CREATE TRIGGER check_xp_completed_trigger BEFORE
INSERT
    OR
UPDATE ON checks FOR EACH ROW EXECUTE FUNCTION check_parent_task_in_xp();
...
DECLARE count_records INTEGER;
BEGIN
SELECT COUNT(*) INTO count_records
FROM p2p
WHERE check_id = NEW.check_id;
IF count_records >= 2 THEN RAISE EXCEPTION 'maximum number of records with the same check_id in p2p reached';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
...
-- Проверяет, что добавляемая запись проверки в таблицу p2p не является третьей
CREATE TRIGGER p2p_check_two_records_trigger BEFORE
INSERT
    OR
UPDATE ON p2p FOR EACH ROW EXECUTE FUNCTION p2p_check_two_records();
...
FROM verter
WHERE check_id = NEW.check_id;
IF count_records >= 2 THEN RAISE EXCEPTION 'maximum number of records with the same check_id in verter reached';
END IF;
RETURN NEW;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что добавляемая запись проверки в таблицу verter не является третьей
CREATE TRIGGER verter_check_two_records_trigger BEFORE
INSERT
    OR
UPDATE ON verter FOR EACH ROW EXECUTE FUNCTION verter_check_two_records();
...
CREATE OR REPLACE FUNCTION check_verter_records_before_delete() RETURNS TRIGGER AS $ $ BEGIN IF (OLD.state_check = 'start') THEN IF EXISTS(
        SELECT 1
        FROM verter
    ...
    ) THEN RAISE EXCEPTION 'сannot delete "start" record from verter with corresponding "success" or "failure" record for the same check_id';
END IF;
END IF;
RETURN OLD;
END;
...
-- Проверяет, что если удаляемая запись содержит статус 'start',
-- то для неё нет записи со статусом 'success' или 'failure'
CREATE TRIGGER trigger_check_verter_records_before_delete BEFORE DELETE ON verter FOR EACH ROW EXECUTE FUNCTION check_verter_records_before_delete();
...
-- то для неё нет записи со статусом 'success' или 'failure'
CREATE OR REPLACE FUNCTION check_p2p_records_before_delete() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
        SELECT 1
        FROM verter
        WHERE check_id = OLD.check_id
    ) THEN RAISE EXCEPTION 'cannot delete record from p2p with existing verter records for the same check_id';
END IF;
IF (OLD.state_check = 'start') THEN IF EXISTS(
    SELECT 1
    FROM p2p
    WHERE check_id = OLD.check_id
        AND (
...
) THEN RAISE EXCEPTION 'сannot delete "start" record from p2p with corresponding "success" or "failure" record for the same check_id';
END IF;
END IF;
RETURN OLD;
END;
...
-- и что если удаляемая запись содержит статус 'start',
-- то для неё нет записи со статусом 'success' или 'failure'
CREATE TRIGGER trigger_check_p2p_records_before_delete BEFORE DELETE ON p2p FOR EACH ROW EXECUTE FUNCTION check_p2p_records_before_delete();
...
CREATE OR REPLACE FUNCTION check_checks_records_before_delete() RETURNS TRIGGER AS $ $ BEGIN IF EXISTS(
        SELECT 1
        FROM verter
    ...
    OR EXISTS(
        ...
        FROM p2p
        WHERE check_id = OLD.id
    ) THEN RAISE EXCEPTION 'cannot delete record from checks with existing verter or p2p records for the same check_id';
END IF;
RETURN OLD;
END;
$ $ LANGUAGE plpgsql;
-- Проверяет, что для удаляемой записи нет проверок p2p и verter
CREATE TRIGGER trigger_check_checks_records_before_delete BEFORE DELETE ON checks FOR EACH ROW EXECUTE FUNCTION check_checks_records_before_delete();
